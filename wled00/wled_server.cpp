...
void initServer()
{
  ...
  server.on("/temperature", HTTP_GET, [](AsyncWebServerRequest *request){
	  WLED::instance().sht.readSample();
      request->send(200, "application/json", String(WLED::instance().sht.getTemperature()));
  });
  
  server.on("/humidity", HTTP_GET, [](AsyncWebServerRequest *request){
	  WLED::instance().sht.readSample();
      request->send(200, "application/json", String(WLED::instance().sht.getHumidity()));
  });
